user='' 			#login details to access your favorites and NSFW wallpapers
password=''
...
threads='100'				# the number of wallpapers downloaded simultaneously,each wallpaper is a separate downloading thread. depends on connection speed, more=faster :D ,117 is the limit not the sky
sorting='toplist' 			#sorting options(random,toplist,relevance,date_added,views,favorites).
search='' 	 			#search ,you can leave empty.
toprange='1M'				# download toplist(if enabled in sorting) for specific period(last day=1d,last 3 days=3d,last week=1w,last month=1M
							#last 3 months =3M,last 6 months=6M, last year=1y).
numpages='4'  	 			#number of pages to download(24 in each page).
startpage='2'				#start downloading from a certain page(none will be downloaded if page does not exist).
...
allowduplicates='True'		# Allow the script to download duplicated wallpapers that are already has been downloaded, Check list.of.downloaded.wall.txt .
